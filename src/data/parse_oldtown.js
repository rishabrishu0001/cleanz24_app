...
const csvPath = path.join(__dirname, 'oldtown_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  // Audience Refinement
  if (aLower === 'men' || nLower.includes('gents') || nLower.includes('sherwani') || nLower.includes('achkan')) {
    audience = 'Men';
  } else if (aLower === 'women' || nLower.includes('ladies') || nLower.includes('saree') || nLower.includes('lehnga') || nLower.includes('lehenga') || nLower.includes('blouse') || nLower.includes('sharara') || nLower.includes('gown') || nLower.includes('anarkali') || nLower.includes('kaftan') || nLower.includes('frock')) {
    audience = 'Women';
  } else if (aLower.includes('house') || nLower.includes('carpet') || nLower.includes('curtain') || nLower.includes('bedsheet') || nLower.includes('quilt') || nLower.includes('blanket') || nLower.includes('pillow') || nLower.includes('cushion') || nLower.includes('towel') || nLower.includes('table cloth') || nLower.includes('soft toy')) {
    audience = 'Household';
  }
  ...
  if (name.toLowerCase().includes('per kg') || rawName.toLowerCase().includes('per kg') || name.toLowerCase().includes('kg')) {
    ...
    unit = '/ pair';
  } else if (name.toLowerCase().includes('sqft') || name.toLowerCase().includes('sq.ft')) {
    unit = '/ sq.ft';
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Old Town Bhubaneswar Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'oldtown_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const OLDTOWN_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'oldtownCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Old Town Bhubaneswar items.`);
