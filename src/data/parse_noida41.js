...
const csvPath = path.join(__dirname, 'noida41_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Noida Sector 41 Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'noida41_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const NOIDA41_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'noida41Catalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Noida Sector 41 items.`);
