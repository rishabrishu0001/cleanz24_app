...
const csvPath = path.join(__dirname, 'kharar_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (cLower.includes('steam') || cLower.includes('si') || cLower.includes('iron') || cLower.includes('press')) {
  ...
  if (catLower.includes('shoe')) audience = 'Footwear';
  else if (catLower.includes('bag')) audience = 'Bags & Luggage';
  else if (catLower.includes('access')) audience = 'Accessories';
  else if (catLower.includes('house') || catLower.includes('religous') || catLower.includes('sofa')) audience = 'Household';
  ...
  else if (nLower.includes('sherwani') || nLower.includes('safari') || nLower.includes('achkan') || nLower.includes('dhoti') || nLower.includes('lungi') || nLower.includes('kurta pajama male') || nLower.includes('turban') || nLower.includes('dastaar')) audience = 'Men';
  ...
  // Unit assignment
  let unit = '/ pc';
  if (nLower.includes('per kg') || nLower.includes('/ kg') || (serviceKey === 'wash_fold' && !nLower.includes('piece') && !nLower.includes('towel') && !nLower.includes('kurta') && !nLower.includes('bedsheet') && !nLower.includes('turban') && !nLower.includes('pcs') && !nLower.includes('fold') && !nLower.includes('iron'))) {
    unit = '/ kg';
  ...
  items.push({
    id: `khr_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Kharar Punjab Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'kharar_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const KHARAR_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'khararCatalog.js'), jsContent);
