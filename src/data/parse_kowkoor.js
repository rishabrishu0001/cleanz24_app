...
const csvPath = path.join(__dirname, 'kowkoor_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  let unit = '/ pc';
  if (name.toLowerCase().includes('per kg') || rawName.toLowerCase().includes('per kg')) {
    unit = '/ kg';
  } else if (name.toLowerCase().includes('pair')) {
    unit = '/ pair';
  } else if (name.toLowerCase().includes('suit') || name.toLowerCase().includes('pcs')) {
    unit = '/ set';
  }
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Kowkoor Secunderabad Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'kowkoor_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const KOWKOOR_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'kowkoorCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Kowkoor items.`);
