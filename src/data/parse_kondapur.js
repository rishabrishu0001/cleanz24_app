...
const csvPath = path.join(__dirname, 'kondapur_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (sLower.includes('press') || sLower.includes('steam') || sLower.includes('iron')) {
  ...
  else if (aLower === 'house hold' || aLower === 'institutional') audience = 'Household';
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Kondapur Hyderabad Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'kondapur_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const KONDAPUR_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'kondapurCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Kondapur items.`);
