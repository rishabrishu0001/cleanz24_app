...
const csvPath = path.join(__dirname, 'bhilwara_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (sLower.includes('steam') || sLower.includes('iron') || sLower.includes('starch')) {
    serviceKey = 'steam_iron';
    service = 'Steam Iron';
    image = '/images/steamiron.jpg';
  } else if (sLower.includes('shoe') || aLower.includes('shoe') || nLower.includes('shoe') || nLower.includes('boots') || nLower.includes('crocs') || nLower.includes('sandels') || nLower.includes('slippers') || nLower.includes('juti')) {
    ...
    image = '/images/drycleaning.jpg';
  } else if (sLower.includes('kid') || aLower.includes('kid')) {
    serviceKey = 'dry_clean';
    service = 'Kids Dry Clean';
    image = '/images/drycleaning.jpg';
    if (!audience || audience === 'Dry Clean') audience = 'Kids';
  }

  // Adjust audience if detected
  if (nLower.includes('kids') || nLower.includes('baby')) {
    audience = 'Kids';
  } else if (nLower.includes('women') || aLower.includes('women')) {
    audience = 'Women';
  } else if (nLower.includes('men') && !nLower.includes('women')) {
    audience = 'Men';
  }
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Bhilwara Rajasthan Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'bhilwara_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const BHILWARA_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'bhilwaraCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Bhilwara items.`);
