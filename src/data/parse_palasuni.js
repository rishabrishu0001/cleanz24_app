...
const csvPath = path.join(__dirname, 'palasuni_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  // Unit assignment
  let unit = '/ pc';
  if (nLower.includes('per pcs') || nLower.includes('per pc')) {
    unit = '/ pc';
  } else if (nLower.includes('per kg') || nLower.includes('/ kg') || (serviceKey === 'wash_fold' && !nLower.includes('pcs') && !nLower.includes('pc'))) {
    unit = '/ kg';
  ...
  items.push({
    id: `pls_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Palasuni Bhubaneswar Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'palasuni_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const PALASUNI_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'palasuniCatalog.js'), jsContent);
