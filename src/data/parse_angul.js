...
const csvPath = path.join(__dirname, 'angul_price_list.csv');
...
// Build fallback lookup from VAISHALI_CATALOG
const fallbackMap = {};
...
for (let i = 1; i < lines.length; i++) {
  ...
  items.push({
    id: `ang_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Angul Odisha Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'angul_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const ANGUL_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'angulCatalog.js'), jsContent);
