...
const csvPath = path.join(__dirname, 'parad_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  items.push({
    id: `prd_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Parad Kannur Kerala Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'parad_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const PARAD_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'paradCatalog.js'), jsContent);
