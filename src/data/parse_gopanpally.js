...
const csvPath = path.join(__dirname, 'gopanpally_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  } else if (sLower.includes('bag') || aLower.includes('bag') || nLower.includes('trolley') || nLower.includes('trolly') || nLower.includes('handbag') || nLower.includes('bagpack')) {
    serviceKey = 'bag_care';
  ...
  if (name.toLowerCase().includes('per kg') || rawName.toLowerCase().includes('per kg') || name.toLowerCase().includes('kg')) {
    ...
    unit = '/ pair';
  } else if (name.toLowerCase().includes('sq.ft') || name.toLowerCase().includes('sqft')) {
    unit = '/ sq.ft';
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Gopanpally Hyderabad Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'gopanpally_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const GOPANPALLY_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'gopanpallyCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Gopanpally items.`);
