...
const csvPath = path.join(__dirname, 'cuttack_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  } else if (sLower.includes('winter') || aLower.includes('winter')) {
  ...
  // Audience Refinement
  if (aLower === 'men' || aLower === 'boy') audience = (aLower === 'boy' || aLower.includes('kids')) ? 'Kids' : 'Men';
  else if (aLower === 'women' || aLower === 'girls' || aLower === 'girl') audience = (aLower === 'girls' || aLower === 'girl' || aLower.includes('kids')) ? 'Kids' : 'Women';
  else if (aLower.includes('kids') || aLower.includes('boy') || aLower.includes('girl')) audience = 'Kids';
  else if (aLower.includes('house') || aLower.includes('institutional')) audience = 'Household';
  else if (nLower.includes('kids') || nLower.includes('child') || nLower.includes('frock')) audience = 'Kids';
  else if (nLower.includes('women') || nLower.includes('ladies') || nLower.includes('saree') || nLower.includes('lehnga') || nLower.includes('lehenga') || nLower.includes('blouse')) audience = 'Women';
  else if (nLower.includes('men') || nLower.includes('gents') || nLower.includes('sherwani') || nLower.includes('achkan')) audience = 'Men';

  let unit = '/ pc';
  if (name.toLowerCase().includes('per kg') || rawName.toLowerCase().includes('per kg') || name.toLowerCase().includes('per kg')) {
    unit = '/ kg';
  } else if (name.toLowerCase().includes('pair')) {
    unit = '/ pair';
  } else if (name.toLowerCase().includes('sq.ft') || name.toLowerCase().includes('sqft') || name.toLowerCase().includes('square feet') || name.toLowerCase().includes('per pennel') || name.toLowerCase().includes('per penel') || name.toLowerCase().includes('per panel')) {
    unit = '/ panel';
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at CDA Cuttack Odisha Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'cuttack_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const CUTTACK_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'cuttackCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Cuttack items.`);
