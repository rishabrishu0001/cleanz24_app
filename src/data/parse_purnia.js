...
const csvPath = path.join(__dirname, 'purnia_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  // Unit assignment
  let unit = '/ pc';
  if (nLower.includes('per kg') || nLower.includes('/ kg') || (serviceKey === 'wash_fold' && !nLower.includes('piece') && !nLower.includes('towel') && !nLower.includes('garment') && !nLower.includes('inner') && !nLower.includes('shoe'))) {
    unit = '/ kg';
  ...
  items.push({
    id: `pur_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Purnia Bihar Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'purnia_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const PURNIA_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'purniaCatalog.js'), jsContent);
