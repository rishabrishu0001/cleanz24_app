...
import { BHILWARA_CATALOG } from './bhilwaraCatalog.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const csvPath = path.join(__dirname, 'udaipur_price_list.csv');
...
// Build fallback lookup with Bhilwara (Rajasthan) & Vaishali
const fallbackMap = {};
BHILWARA_CATALOG.forEach(item => {
...
for (let i = 1; i < lines.length; i++) {
  ...
  name = name.split(' ').map(w => {
    if (w === '-' || w === '/' || w === '&' || w.startsWith('(') || w.endsWith(')') || w.startsWith('{') || w.endsWith('}')) return w;
  ...
  } else if (cLower.includes('shoe') || catLower.includes('shoe') || nLower.includes('shoe') || nLower.includes('sneaker') || nLower.includes('boot') || nLower.includes('sandel') || nLower.includes('slipper') || nLower.includes('crocs')) {
  ...
  else if (nLower.includes('poshak') || nLower.includes('ladies') || nLower.includes('saree') || nLower.includes('lehnga') || nLower.includes('lehenga') || nLower.includes('blouse') || nLower.includes('dupatta') || nLower.includes('gown') || nLower.includes('sarara') || nLower.includes('sharara') || nLower.includes('skirt') || nLower.includes('petti coat') || nLower.includes('bra') || nLower.includes('shrug') || nLower.includes('choli') || nLower.includes('rida')) audience = 'Women';
  else if (nLower.includes('saafa') || nLower.includes('sherwani') || nLower.includes('safari') || nLower.includes('achkan') || nLower.includes('dhoti') || nLower.includes('lungi') || nLower.includes('kurta') || nLower.includes('turban') || nLower.includes('tuxedo')) audience = 'Men';
  ...
  if (nLower.includes('per kg') || nLower.includes('/ kg') || serviceKey === 'wash_fold') {
    ...
    unit = '/ panel';
  } else if (nLower.includes('suit') || nLower.includes('poshak') || nLower.includes('cord set') || nLower.includes('pcs') || nLower.includes('set') || nLower.includes('pec') || nLower.includes('suite')) {
    unit = '/ set';
  }
  ...
  if (isNaN(rate) || rate <= 0) {
    ...
    if (fallbackMap[key]) {
      ...
      zeroPriceCount++;
      if (norm.includes('poshak4') || norm.includes('poshakheavy')) rate = 499;
      else if (norm.includes('poshak')) rate = 399;
      else if (norm.includes('saafa') && serviceKey === 'steam_iron') rate = 30;
      else if (norm.includes('saafa')) rate = 99;
      else if (serviceKey === 'steam_iron') rate = 20;
  ...
  items.push({
    id: `udp_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Udaipur Panchwati Rajasthan Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'udaipur_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const UDAIPUR_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'udaipurCatalog.js'), jsContent);
