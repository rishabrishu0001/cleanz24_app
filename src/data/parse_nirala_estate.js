...
const csvPath = path.join(__dirname, 'nirala_estate_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  const rawCatalog = parts[11].trim(); // Dry Clean, Laundry, Shoe & Bag Clean, Winter Wear, Steam Iron, Winter Wear SI
  const rawCategory = parts[12].trim(); // Upper Wear, Full Body, Bottom Wear, Laundry, Shoes Cleaning, Bag Cleaning, Winter, Winters, House Hold
  ...
  else if (nLower.includes('child') || nLower.includes('kids') || nLower.includes('frock')) audience = 'Kids';
  else if (nLower.includes('ladies') || nLower.includes('saree') || nLower.includes('lehnga') || nLower.includes('lehenga') || nLower.includes('blouse') || nLower.includes('dupatta') || nLower.includes('gown') || nLower.includes('sarara') || nLower.includes('sharara') || nLower.includes('skirt') || nLower.includes('petti coat')) audience = 'Women';
  else if (nLower.includes('sherwani') || nLower.includes('safari') || nLower.includes('achkan') || nLower.includes('dhoti') || nLower.includes('lungi')) audience = 'Men';
  else if (catLower.includes('upper') || catLower.includes('bottom') || catLower.includes('full body') || catLower.includes('winter')) {
    if (nLower.includes('blazer') || nLower.includes('suit') || nLower.includes('coat') || nLower.includes('jacket') || nLower.includes('shirt') || nLower.includes('pant') || nLower.includes('jeans')) {
      audience = 'Men & Women';
    } else {
      audience = 'Men & Women';
    }
  ...
  items.push({
    id: `nre_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Nirala Estate Greater Noida West Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'nirala_estate_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const NIRALA_ESTATE_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'niralaEstateCatalog.js'), jsContent);
