...
const csvPath = path.join(__dirname, 'siliguri_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (catLower === 'men') audience = 'Men';
    ...
    if (nLower.includes('shirt') || nLower.includes('suit') || nLower.includes('kurta') || nLower.includes('sherwani') || nLower.includes('jacket') || nLower.includes('kameez') || nLower.includes('overcoat') || nLower.includes('payjama')) {
      audience = 'Men';
    } else {
      audience = 'Household';
    }
  ...
  if (nLower.includes('sq.ft') || nLower.includes('/sq.ft')) {
    unit = '/ sq.ft';
  } else if (nLower.includes('per kg') || nLower.includes('/ kg') || cLower.includes('by weight') && !nLower.includes('per pcs') && !nLower.includes('per pc')) {
    unit = '/ kg';
  ...
  items.push({
    id: `slg_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Siliguri West Bengal Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'siliguri_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const SILIGURI_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'siliguriCatalog.js'), jsContent);
