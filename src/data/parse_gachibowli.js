...
const csvPath = path.join(__dirname, 'gachibowli_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Gachibowli Hyderabad Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'gachibowli_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const GACHIBOWLI_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'gachibowliCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Gachibowli items.`);
