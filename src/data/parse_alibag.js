...
const csvPath = path.join(__dirname, 'alibag_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  // Audience Refinement
  if (aLower === 'men' || aLower === 'boy') audience = aLower === 'boy' ? 'Kids' : 'Men';
  else if (aLower === 'women' || aLower === 'girls' || aLower === 'girl') audience = (aLower === 'girls' || aLower === 'girl') ? 'Kids' : 'Women';
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Alibag Maharashtra Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'alibag_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const ALIBAG_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'alibagCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Alibag items.`);
