...
import { AMRITSAR_CATALOG } from './amritsarCatalog.js';
import { KHARAR_CATALOG } from './khararCatalog.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const csvPath = path.join(__dirname, 'patiala_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (cLower.includes('steam') || cLower.includes('iron') || cLower.includes('press') || cLower.endsWith(' si') || cLower.includes('wear si')) {
  ...
  } else if (cLower.includes('bag') || catLower.includes('bag') || nLower.includes('bag') || nLower.includes('trolley') || catLower.includes('sofa clean') || nLower.includes('sofa cleaning')) {
    serviceKey = 'bag_care';
  ...
  else if (catLower.includes('religous') || catLower.includes('religious') || nLower.includes('rumal') || nLower.includes('granth sahib')) audience = 'Religious & Ceremonial';
  ...
  if (isNaN(rate) || rate <= 0) {
    ...
    if (fallbackMap[key]) {
      ...
      else if (serviceKey === 'shoe_care') rate = 349;
  ...
  items.push({
    id: `ptl_${originalId || items.length + 1}`,
    originalId,
    name,
    service,
    serviceKey,
    ...
    desc: `${service} for ${name} (${audience}) at Patiala Punjab Studio`,
    image
  });
}
...
// Write JSON
fs.writeFileSync(path.join(__dirname, 'patiala_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const PATIALA_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'patialaCatalog.js'), jsContent);
