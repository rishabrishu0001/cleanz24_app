...
const csvPath = path.join(__dirname, 'sanchore_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  else if (aLower === 'kids' || aLower.includes('kids') || aLower.includes('boy') || aLower.includes('girl')) audience = 'Kids';
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Sanchore Rajasthan Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'sanchore_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const SANCHORE_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'sanchoreCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Sanchore items.`);
