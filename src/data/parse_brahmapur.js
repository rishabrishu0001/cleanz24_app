...
const csvPath = path.join(__dirname, 'brahmapur_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Brahmapur Berhampur Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'brahmapur_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const BRAHMAPUR_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'brahmapurCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Brahmapur items.`);
