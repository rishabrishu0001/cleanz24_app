...
import { KAZHAKOOTAM_CATALOG } from './kazhakootamCatalog.js';

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

const csvPath = path.join(__dirname, 'thampanoor_price_list.csv');
...
// Build fallback lookup from Kazhakootam (Kerala catalog)
const fallbackLookup = {};
KAZHAKOOTAM_CATALOG.forEach(item => {
...
for (let i = 1; i < lines.length; i++) {
  ...
  else if (aLower.includes('house') || aLower.includes('institutional') || aLower.includes('other') || aLower.includes('b2b')) audience = 'Household';
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Thampanoor Trivandrum Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'thampanoor_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const THAMPANOOR_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'thampanoorCatalog.js'), jsContent);

console.log(`Successfully generated ${items.length} items for Thampanoor Trivandrum Studio.`);
