...
const csvPath = path.join(__dirname, 'bhopal_price_list.csv');
...
for (let i = 1; i < lines.length; i++) {
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Bhopal MP Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'bhopal_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const BHOPAL_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'bhopalCatalog.js'), jsContent);

console.log(`Successfully parsed ${items.length} Bhopal items.`);
