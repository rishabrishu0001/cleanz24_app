...
const csvPath = path.join(__dirname, 'roorkee_price_list.csv');
...
// Build fallback lookup from Vaishali / NCR catalog
...
for (let i = 1; i < lines.length; i++) {
  ...
  if (isNaN(rate) || rate <= 0) {
    ...
    if (fallbackLookup[normKey]) {
      ...
      // Sensible standard rates if unmapped
  ...
  items.push({
    ...
    desc: `${service} for ${name} (${audience}) at Roorkee Uttarakhand Studio`,
    image
  });
}

// Write JSON
fs.writeFileSync(path.join(__dirname, 'roorkee_prices.json'), JSON.stringify(items, null, 2));

// Write JS export
const jsContent = `export const ROORKEE_CATALOG = ${JSON.stringify(items, null, 2)};\n`;
fs.writeFileSync(path.join(__dirname, 'roorkeeCatalog.js'), jsContent);

console.log(`Successfully generated ${items.length} items for Roorkee Uttarakhand Studio.`);
